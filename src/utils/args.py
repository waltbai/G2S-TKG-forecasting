...
import llamafactory.hparams as hp
...
class AnonymizedDataArguments(hp.DataArguments):
    ...
    anonymizer: str = "session"
    anonymize_entity: bool = True
    anonymize_rel: bool = True
    time_processor: str = "query"
    prompt_construct_strategy: str = "qa"
...
class DeAnonymizedDataArguments(hp.DataArguments):
    ...
    history_direction: str = "uni"
    history_length: int = 30
    anonymizer: str = "global"
    anonymize_entity: bool = True
    anonymize_rel: bool = True
    time_processor: str = "start"
    deanonymize_strategy: str = "map"
    prompt_construct_strategy: str = "qa"
...
def get_train_args(stage: str = "stage1"):
    if stage == "stage1":
        parser = HfArgumentParser(STAGE1_ARGS)
    else:
        parser = HfArgumentParser(STAGE2_ARGS)
...
def get_infer_args(stage: str = "stage1"):
    if stage == "stage1":
        parser = HfArgumentParser(STAGE1_ARGS)
    else:
        parser = HfArgumentParser(STAGE2_ARGS)
